...
public class SalesPromiseRepository {
    ...
    public List<SalesPromiseDTO> findAll() {
        String sql = "SELECT id, name, sale_date, sale_value, real_estate_number, credit, credit_disbursement_date, credit_value, bank_id, agent_id, customer_id, real_estate_id FROM SalesPromise";
        return jdbcTemplate.query(sql, (rs, rowNum) -> {
            SalesPromiseDTO salesPromise = new SalesPromiseDTO();
            salesPromise.setId(rs.getInt("id"));
            salesPromise.setName(rs.getString("name"));
    ...
    public SalesPromiseDTO findById(Integer id) {
        String sql = "SELECT id, name, sale_date, sale_value, real_estate_number, credit, credit_disbursement_date, credit_value, bank_id, agent_id, customer_id, real_estate_id FROM SalesPromise WHERE id = ?";
    ...
    public void save(SalesPromiseDTO salesPromise) {
        String sql = "INSERT INTO SalesPromise (name, sale_date, sale_value, real_estate_number, credit, credit_disbursement_date, credit_value, bank_id, agent_id, customer_id, real_estate_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, salesPromise.getName(), salesPromise.getSaleDate(), salesPromise.getSaleValue(), salesPromise.getRealEstateNumber(), salesPromise.getCredit(), salesPromise.getCreditDisbursementDate(), salesPromise.getCreditValue(), salesPromise.getBank().getId(), salesPromise.getAgent().getId(), salesPromise.getCustomer().getId(), salesPromise.getRealEstate().getId());
    }

    public void update(Integer id, SalesPromiseDTO salesPromise) {
        String sql = "UPDATE SalesPromise SET name = ?, sale_date = ?, sale_value = ?, real_estate_number = ?, credit = ?, credit_disbursement_date = ?, credit_value = ?, bank_id = ?, agent_id = ?, customer_id = ?, real_estate_id = ? WHERE id = ?";
